...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    include "C:\Users\VICTUS\Desktop\Folders\languages\assembly\finalgame\macro.h"
    include "C:\Users\VICTUS\Desktop\Folders\languages\assembly\finalgame\vcs.h"
...
MissileYPos     byte         ; missile x-position
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    MAC DRAW_MISSILE             ; Macro to check if we should draw the missile
        lda #%00000000
        cpx MissileYPos          ; Compare X (current scanline) with missile Y pos
        bne .SkipMissileDraw     ; if x is not equal to missile Y position, then skip draw
.DrawMissile:
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    lda MissileXPos
    ldy #2                  ; 2 will point to missle
    jsr SetObjectXPos       ; Set misseles horizontal position
...
CheckButtonPressed:         ; Missile Check
...
.SetScoreValues:
    ;BCD changes binaries to decimals

    sed                     ; Set decimal mode for score and timer variables

    lda Timer
    clc
    adc #1
    sta Timer       ; Add 1 to timer (Because BCD (decimal mode) doesnt like INC)
...
CheckCollisionP0P1:
    ...
    jmp CheckCollisionMOP1    ; else, skip to next check
.P0P1Collided:
    jsr GameOver             ; call GameOver subroutine
CheckCollisionMOP1:
    lda #%10000000
    bit CXM0P               ; Check if a collision happened between p1 and missile
    bne .MOP1Collilded
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Enable the jet engine sound based on the jet-y position
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
GenerateJetSound subroutine
    ...
    ;frequency
    lda JetYPos         ; Load a with JETY
    lsr
    lsr
    lsr                 ; Divide by 8 to get a number in range 1-31. (Because frequency needs to be in that interval)
    sta Temp
    lda #30             ; High y position should have a higher pitch so to reverse it, we subtract 31 from the result
    sec
    sbc Temp
    sta AUDF0           ; Apply the result to frequency memory
...
GenerateMissileSound subroutine
    ...
    ;frequency
    lda #10             ; High y position should have a higher pitch so to reverse it, we subtract 31 from the result
    sta AUDF0           ; Apply the result to frequency memory
